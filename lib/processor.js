...
function preprocess(text, filename) {
  ...
  while ((match = regex.erbRegexChained.exec(text)) !== null) {
    ...
    if (offsetLookup.has(key)) {
      ...
      const hightestKey = Math.max(...Array.from(exisitingMap.keys()));
      const colDiffHighestKey = exisitingMap.get(hightestKey)[1];
      newPartialMap.set(endIndexProcessed, [numAddLinesCounter, numAddColumns + colDiffHighestKey]);
